...
class SearchUsersBloc extends Bloc<LoadFriendEvent, LoadedFriendsState> {
  SearchUsersBloc() : super((const LoadedFriendsState())) {
  ...
  Future _onInitial(InitialEvent event, Emitter emit) async {
    try {
      ...
      final friends = await useCase.searchUsers(event.searchQuery, 1, 5);
  ...
  Future _onRefreshFriends(RefreshFriendsEvent event, Emitter emit) async {
    ...
    try {
      ...
      final useCase = await getIt.getAsync<FriendUseCase>();
      final friends = await useCase.searchUsers('', 1, 5);
