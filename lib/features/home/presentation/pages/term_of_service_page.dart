...
class _TermsOfServicePageState extends State<TermsOfServicePage> {
  ...
  Future<void> _loadAcceptance() async {
    //final accepted = await HiveService.getTerms();
    final accepted = true;
    setState(() {
      _isAccepted = accepted ?? false;
      _isLoading = false;
    });
  }

  Future<void> _acceptTerms() async {
    //await HiveService.saveTerms(true);
    setState(() {
      _isAccepted = true;
    });
  }

  Future<void> _declideTerms() async {
    //await HiveService.saveTerms(false);
    setState(() {
      _isAccepted = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Example of formatting dates with Intl
    // final effectiveDate = DateFormat.yMMMMd(
    //   intl.localeOf(context).languageCode,
    // ).format(DateTime(2025, 7, 30));
    // final lastUpdatedDate = DateFormat.yMMMMd(
    //   intl.localeOf(context).languageCode,
    // ).format(DateTime(2025, 7, 30));
    ...
    return Scaffold(
      appBar: AppBar(title: Text(intl.termsOfServiceTitle)),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // ALobo Sports Hub title
            Center(
              // Centered the main title
              child: Text(intl.aloboSportsHub, style: titleMedium),
            ),
            ...
            Center(
              child: Text(
                intl.effectiveDate(effectiveDate),
                style: bodySmall,
              ),
            ...
            const SizedBox(height: 8.0),
            Text(intl.aloboDescription, style: bodySmall),
            const SizedBox(height: 8.0),
            _buildBulletPoint(context, intl.profileCreation),
            _buildBulletPoint(context, intl.postContent),
            _buildBulletPoint(context, intl.socialInteractions),
            _buildBulletPoint(context, intl.searchUsers),
            _buildBulletPoint(context, intl.createJoinGroups),
            _buildBulletPoint(context, intl.organizeEvents),
            _buildBulletPoint(context, intl.receiveNotifications),
            _buildBulletPoint(context, intl.earnAchievements),
            const SizedBox(height: 24.0),
            ...
            _buildBulletPoint(context, intl.createShareContent),
            const SizedBox(height: 16.0),
            ...
            _buildBulletPoint(context, intl.copyrightViolation),
            ...
            _buildBulletPoint(context, intl.aloboContentRights),
            const SizedBox(height: 16.0),
            ...
            _buildBulletPoint(context, intl.postInteractions),
            _buildBulletPoint(context, intl.locationData),
            ...
            _buildBulletPoint(context, intl.createProfilesPosts),
            _buildBulletPoint(context, intl.basicSocialInteractions),
            _buildBulletPoint(context, intl.joinGroupsEvents),
            _buildBulletPoint(context, intl.userSearch),
            const SizedBox(height: 16.0),
            ...
            const SizedBox(height: 8.0),
            Text(intl.alobosRights, style: titleSmall),
            const SizedBox(height: 8.0),
            ...
            const SizedBox(height: 16.0),
            Text(intl.terminationByAlobo, style: titleSmall),
            const SizedBox(height: 8.0),
            ...
            _buildBulletPoint(
              context,
              intl.noResponsibilityUserContent,
            ),
            _buildBulletPoint(context, intl.noGuaranteeAccuracy),
            _buildBulletPoint(
              context,
              intl.noResponsibilityIndirectDamages,
            ),
            const SizedBox(height: 24.0),
            ...
            _buildBulletPoint(
              context,
              intl.continuedUseMeansAgreement,
            ),
            ...
            _buildBulletPoint(
              context,
              intl.email('support@alobosports.com'),
            ),
            _buildBulletPoint(
              context,
              intl.address(
            ...
            Center(
              child: Text(
                intl.lastUpdated(lastUpdatedDate),
                style: bodySmall,
              ),
            ...
            if (!_isAccepted)
              Center(
                child: ElevatedButton(
                  ...
                  child: Text(intl.accept),
                ),
              ),
            if (_isAccepted)
              Center(
                child: ElevatedButton(
                  onPressed: _declideTerms,
                  child: Text(intl.decline),
                ),
              ),
          ],
        ),
